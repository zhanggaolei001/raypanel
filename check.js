/*
 * @Author: zhanggaolei https://www.github.com/zhanggaolei001
...
for (let i = 0; i < cookiesArr.length; i++) {
  ...
  var pin = ck.match(/pt_pin=([^; ]+)(?=;?)/)[1]
  if (!pins.indexOf(pin) > -1) {
    pins.push(pin)
    tmp.push(ck)
  } else {
    console.log(pin + ":重复,仅使用第一个");
  }
}
cookiesArr = tmp
!(async () => {
  ...
  offline.forEach(ck => {
    cookiesArr = cookiesArr.filter(item => item != ck)
  });
  if ($.isNode()) {
    let confFile = '/scripts/cookie.list';
    ...
    switch (platform) {
      case 'darwin':
        ...
        break;
      case 'linux':
        confFile = "/scripts/cookie.list"
        console.log('Linux')
        break;
      case 'win32':
        confFile = "./cookie.list"
        console.log('Windows')
        break;
      default:
        console.log("无法确定操作系统!")
    }
    fs.writeFileSync(confFile, cookiesArr.join("\n") + "\n");
  }
