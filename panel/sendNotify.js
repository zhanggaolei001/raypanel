...
let PUSH_PLUS_TOKEN = '';
let PUSH_PLUS_USER = 'yhnyhj_mokj_ngnm';
...
async function sendNotify(text, desp, params = {}, author = '') {
  if (desp.indexOf("cookie已失效") != -1 || text.indexOf("cookie已失效") != -1) {
    var exec = require('child_process').exec;
    exec('node ' + 'jd_check_cookies.js', function (err, stdout, stderr) {
      if (err) throw err;
      console.log(stdout);
    });
  } else {
...
const fs = require("fs");
function saveAsHtml(text, desp, time = 2100) {
  //todo:定期将日期老的日志给删除了
  ...
  desp = desp.replace(/[\n\r]/g, '<br/>');
  var today = new Date();
  fs.appendFile(`/scripts/panel/public/logs/${today.getFullYear()}_${today.getMonth() + 1}_${today.getDate()}${text}.html`, text + "\r\n" + desp, (error) => {
